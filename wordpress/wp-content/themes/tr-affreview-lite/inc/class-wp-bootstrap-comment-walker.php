		...
		<<?php echo esc_attr( $tag ); ?> id="comment-<?php comment_ID(); ?>" <?php comment_class( $this->has_children ? 'parent media' : 'media' ); ?>>
			...
			<div class="media-body" id="div-comment-<?php comment_ID(); ?>">
				...
				<div class="comment-metadata">
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID, $args ) ); ?>">
						<time datetime="<?php comment_time( 'c' ); ?>">
							...
							printf( esc_html_x( '%1$s at %2$s', '1: date, 2: time' , 'tr-affreview-lite'), get_comment_date(), get_comment_time() ); ?>
						</time>
					</a>
				</div><!-- .comment-metadata -->

				<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation label label-info"><?php esc_html_e( 'Your comment is awaiting moderation.' , 'tr-affreview-lite'); ?></p>
				<?php endif; ?>				
				...
				<ul class="list-inline">
					<?php edit_comment_link( __( 'Edit' , 'tr-affreview-lite' ), '<li class="edit-link">', '</li>' ); ?>
