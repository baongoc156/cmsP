...
class Shapro_Customizer_Notify_Section extends WP_Customize_Section {
	
	public $type = 'shapro-customizer-notify-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'shapro_cust_notify_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'shapro_cust_notify_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		$shapro_customizer_notify_show_recommended_actions = get_option( 'shapro_customizer_notify_show' );
		foreach ( $shapro_customizer_notify_recommended_actions as $key => $shapro_lite_customizer_notify_recommended_action ) {
			if ( $shapro_customizer_notify_show_recommended_actions[ $shapro_lite_customizer_notify_recommended_action['id'] ] === false ) {
				continue;
			}
			if ( $shapro_lite_customizer_notify_recommended_action['check'] ) {
				continue;
			}

			$shapro_lite_customizer_notify_recommended_action['index'] = $key + 1;

			if ( isset( $shapro_lite_customizer_notify_recommended_action['plugin_slug'] ) ) {
				$active = $this->check_active( $shapro_customizer_notify_recommended_action['plugin_slug'] );
				$shapro_lite_customizer_notify_recommended_action['url'] = $this->create_action_link( $active['needs'], $shapro_lite_customizer_notify_recommended_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$shapro_lite_customizer_notify_recommended_action['class'] = 'active';
				} else {
					$shapro_lite_customizer_notify_recommended_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$shapro_lite_customizer_notify_recommended_action['button_class'] = 'install-now button';
						$shapro_lite_customizer_notify_recommended_action['button_label'] = $install_button_label;
						break;
					case 'activate':
						$shapro_lite_customizer_notify_recommended_action['button_class'] = 'activate-now button button-primary';
						$shapro_lite_customizer_notify_recommended_action['button_label'] = $activate_button_label;
						break;
					case 'deactivate':
						$shapro_lite_customizer_notify_recommended_action['button_class'] = 'deactivate-now button';
						$shapro_lite_customizer_notify_recommended_action['button_label'] = $shapro_deactivate_button_label;
						break;
				}
			}
			$formatted_array[] = $shapro_lite_customizer_notify_recommended_action;
		}// End foreach().

		$customize_plugins = array();

		$shapro_lite_customizer_notify_show_recommended_plugins = get_option( 'shapro_customizer_notify_show_recommended_plugins' );

		foreach ( $shapro_customizer_notify_recommended_plugins as $slug => $plugin_opt ) {

			if ( ! $plugin_opt['recommended'] ) {
				continue;
			}

			if ( isset( $shapro_lite_customizer_notify_show_recommended_plugins[ $slug ] ) && $shapro_lite_customizer_notify_show_recommended_plugins[ $slug ] ) {
				continue;
			}
			...
			switch ( $active['needs'] ) {
				...
				case 'deactivate':
					...
					$ti_customizer_notify_recommended_plugin['button_label'] = $shapro_deactivate_button_label;
					break;
			}
		...
		$json['total_actions']       = count( $shapro_customizer_notify_recommended_actions );
		...
		<# if( data.recommended_actions.length > 0 || data.recommended_plugins.length > 0 ){ #>
			<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
				...
				<h3 class="accordion-section-title">
					...
					<# if( data.recommended_actions.length > 0 ){ #>
						<span class="shapro-customizer-plugin-notify-actions-count">
							<span class="current-index">{{ data.recommended_actions[0].index }}</span>
							{{ data.total_actions }}
						</span>
					<# } #>
				</h3>
				<div class="shapro-theme-recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_actions.length > 0 ){ #>
						<# for (action in data.recommended_actions) { #>
							<div class="shapro-recommeded-actions-container epsilon-required-actions" data-index="{{ data.recommended_actions[action].index }}">
								<# if( !data.recommended_actions[action].check ){ #>
									<div class="shapro-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_actions[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no shapro-customizer-notify-dismiss-recommended-action" id="{{ data.recommended_actions[action].id }}"></span>
					...
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="shapro-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="shapro-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_plugins[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no shapro-customizer-notify-dismiss-button-recommended-plugin" id="{{ data.recommended_plugins[action].id }}"></span>
